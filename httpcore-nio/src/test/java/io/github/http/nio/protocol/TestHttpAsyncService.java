...
package io.github.http.nio.protocol;
...
import io.github.http.concurrent.Cancellable;
import io.github.http.impl.DefaultHttpResponseFactory;
import io.github.http.message.BasicHttpEntityEnclosingRequest;
import io.github.http.message.BasicHttpRequest;
import io.github.http.message.BasicHttpResponse;
...
import io.github.http.nio.NHttpConnection;
import io.github.http.nio.NHttpServerConnection;
import io.github.http.nio.entity.NStringEntity;
import io.github.http.nio.protocol.HttpAsyncService.Incoming;
import io.github.http.nio.protocol.HttpAsyncService.Outgoing;
import io.github.http.nio.protocol.HttpAsyncService.PipelineEntry;
import io.github.http.nio.protocol.HttpAsyncService.State;
import io.github.http.nio.reactor.SessionBufferStatus;
...
public class TestHttpAsyncService {
    ...
    public void testExceptionHandlingRuntimeException() throws Exception {
        ...
        Mockito.doThrow(new RuntimeException()).when(this.httpProcessor).process(
                any(HttpResponse.class), any(HttpContext.class));
        final HttpException httpex = new HttpException();
        try {
            this.protocolHandler.exception(this.conn, httpex);
    ...
    public void testBasicRequest() throws Exception {
        ...
        Mockito.when(this.requestHandler.processRequest(
                eq(request), any(HttpContext.class))).thenReturn(this.requestConsumer);
        ...
        Mockito.verify(this.httpProcessor).process(eq(request), argumentCaptor.capture());
    ...
    public void testRequestPipelineIfResponseInitiated() throws Exception {
        ...
        Mockito.verify(this.requestConsumer).requestReceived(request);
        Mockito.verify(this.requestConsumer).requestCompleted(any());
        Mockito.verify(this.requestHandler, Mockito.never()).handle(
                any(),
                any(HttpAsyncExchange.class),
    ...
    public void testEntityEnclosingRequestContinueWithoutVerification() throws Exception {
        ...
        Mockito.verify(this.requestConsumer).requestReceived(request);
        Mockito.verify(this.conn, Mockito.never()).suspendInput();
        Mockito.verify(this.conn).submitResponse(argThat(argument -> {
    ...
    public void testResponseHandlePipelinedRequest() throws Exception {
        ...
        Mockito.verify(this.requestHandler).handle(same(request),
                argCaptor.capture(), same(exchangeContext));
    ...
    public void testResponseHandleFailedPipelinedRequest() throws Exception {
        ...
        Mockito.verify(this.requestHandler, Mockito.never()).handle(any(),
                any(), any());
        Mockito.verify(this.conn).submitResponse(same(response));
    }
    ...
    public void testTimeoutClosingConnection() throws Exception {
        ...
        Mockito.verify(this.conn).shutdown();
        Mockito.verify(this.requestConsumer).failed(any(SocketTimeoutException.class));
        Mockito.verify(this.requestConsumer).close();
        Mockito.verify(this.responseProducer).failed(any(SocketTimeoutException.class));
        Mockito.verify(this.responseProducer).close();
    }
